...
const noParam = ['waylineId', 'executeHeightMode', 'waypointSpeed', 'executeHeight']

/**
 * Converts input data into a KML template string.
 * @param {shipForm} data - The name of the KML object.
 * @returns {string} - The formatted KML string.
 */
function toTemplateKml(data: shipForm) {
...
function toXmlText(key: string, data: any, indentLevel: number = 0) {
    ...
    if ((key == 'imageFormat' || key == 'payloadLensIndex') && Array.isArray(data)) {
