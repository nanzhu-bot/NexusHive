...
if(!function_exists('publish')){
    /**
     * 发布消息到MQTT服务器
     * @param array $message 要发布的消息
     * @param bool $async 是否异步发送（不等待响应）
     * @return bool|string 成功返回true或消息ID，失败返回false
     */
    function publish($message, $async = true)
    {
        global $publish_socket_pool;
        
        if(empty($message)) {
            Log::error('Publish message is empty');
            return false;
        }
        ...
        try {
            // 尝试从连接池获取连接
            $client = null;
            $pool_key = 'tcp://127.0.0.1:1884';
            
            if (isset($publish_socket_pool[$pool_key]) && is_resource($publish_socket_pool[$pool_key])) {
                $client = $publish_socket_pool[$pool_key];
            } else {
                // 创建新连接
                $client = stream_socket_client('tcp://127.0.0.1:1884', $errno, $errmsg, 1);
                if(!$client){
                    Log::error('Failed to connect to MQTT server: ' . $errmsg);
                    return false;
                }
                ...
                // 保存到连接池
                $publish_socket_pool[$pool_key] = $client;
            }
            
            // 发送数据
            $data = json_encode($message) . "\n";
            $bytes_written = fwrite($client, $data);
            
            if ($bytes_written === false || $bytes_written < strlen($data)) {
                Log::error('Failed to write data to MQTT server');
                // 连接可能已失效，从池中移除
                if (isset($publish_socket_pool[$pool_key])) {
                    fclose($publish_socket_pool[$pool_key]);
                    unset($publish_socket_pool[$pool_key]);
                }
                return false;
            }
            ...
            if($numChanged === false){
                Log::error('Stream select error');
            } elseif($numChanged === 0){
                Log::error('Stream select timeout');
            }
            
            $_ok = trim(fread($client, 8192));
            
            // 检查响应
            if($_ok === 'ok'){
                return true;
            } else {
                Log::error('MQTT server returned error: ' . $_ok);
                return false;
            }
        } catch (\Exception $e) {
...
if(!function_exists('publishOther')){
    function publishOther($message)
    {
        Log::write('publishOther function called with message: '.var_export($message, true));
        if(empty($message)) {
            Log::write('publishOther failed: message is empty');
            return false;
        }
        try {
            Log::write('Trying to connect to tcp://127.0.0.1:1885');
            $client = stream_socket_client('tcp://127.0.0.1:1885', $errno, $errmsg, 1);
            if(!$client){
                Log::write('Connection failed: '.$errmsg.' (errno: '.$errno.')');
                return false;
            }
            Log::write('Connection established successfully');
            // 设置非阻塞模式
            stream_set_blocking($client, false);
            // 发送数据
            $data = json_encode($message) . "\n";
            fwrite($client, $data);
            Log::write('Sent data: '.$data);
            // 使用select实现超时读取
            $read = [$client];
            $write = null;
            $except = null;
            $timeout = 3; // 3秒超时
            Log::write('Waiting for response with timeout: '.$timeout.'s');
            $numChanged = stream_select($read, $write, $except, $timeout);
            if($numChanged === false){
                Log::write('stream_select failed');
                fclose($client);
                return false;
            }elseif($numChanged === 0){
                Log::write('Response timeout after '.$timeout.'s');
                fclose($client);
                return false;
            }else{
                $_ok = trim(fread($client, 8192));
                Log::write('Received response: '.$_ok);
                fclose($client);
                if($_ok == 'ok'){
                    Log::write('publishOther successful');
                    return true;
                }else{
                    Log::write('publishOther failed: invalid response');
                    return false;
                }
            }
        }catch (\ErrorException $e){
            Log::write('publishOther exception: '.$e->getMessage());
            return false;
        }
    }
}
