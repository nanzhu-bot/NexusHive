    ...
    >
        ...
        <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
            ...
            >
                ...
                >
                    <FormItem
                        :label="t('project.user.project_id')"
                        type="remoteSelect"
                        v-model="baTable.form.items!.project_id"
                        prop="project_id"
                        :input-attr="{ pk: 'project.id', field: 'name', remoteUrl: '/admin/Project/index' }"
                        :placeholder="t('Please select field', { field: t('project.user.project_id') })"
                    />
                    <FormItem
                        :label="t('project.user.admin_id')"
                        type="remoteSelect"
                        v-model="baTable.form.items!.admin_id"
                        prop="admin_id"
                        :input-attr="{ pk: 'admin.id', field: 'username', remoteUrl: '/admin/auth.Admin/index' }"
                        :placeholder="t('Please select field', { field: t('project.user.admin_id') })"
                    />
                    <FormItem
                        :label="t('project.user.status')"
                        type="switch"
                        v-model="baTable.form.items!.status"
                        prop="status"
                        :input-attr="{ content: { '0': t('project.user.status 0'), '1': t('project.user.status 1') } }"
                    />
                </el-form>
            </div>
        </el-scrollbar>
